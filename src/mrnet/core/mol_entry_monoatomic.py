...
from mrnet.src.mrnet.utils.constants import ROOM_TEMP
...
class MoleculeEntry(MSONable):
    ...
    def from_molecule_document(
        ...
        try:
            # print(mol_doc)
            if isinstance(mol_doc["initial_molecule"], Molecule):
                molecule = mol_doc["initial_molecule"]
            else:
                molecule = Molecule.from_dict(mol_doc["initial_molecule"])  # type: ignore
            energy = mol_doc["final_energy"]
            # enthalpy = mol_doc["enthalpy"]
            # entropy = mol_doc["entropy"]
            entry_id = task_id
